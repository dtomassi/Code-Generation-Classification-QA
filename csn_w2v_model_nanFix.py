...
#word_index = dict(list(reversed(sorted(list(word_index.items()))))
list(word_index.items())[:15]
#print(word_index.items()[:15])
...
for word, index in word2idx_outputs.items():
  try:
    embedding_vector = csn_model_w2v.get_vector(word)
  except KeyError:
...
def code_w2v(sequence, _embedding_matrix = code_embedding_matrix):
  ...
  np.seterr(divide='ignore', invalid='ignore')#ADDED
  return avg_embed/num_nonzero
...
'''
print(np.argwhere(np.isnan(train_intent_embeds)))
print(np.argwhere(np.isnan(train_code_embeds)))
