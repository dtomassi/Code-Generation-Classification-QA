...
print("TOTAL TRAIN SET LENGTH: ",len(df_train_all))
...
print("FINAL TRAINING LENGTH:",len(df_train))
print("VALIDATION SET LENGTH:",len(df_val))

print(df_train['class'].value_counts(),'distribution counts across the train set')

print(df_val['class'].value_counts(),' distribution counts across the val set')

print(df_test['class'].value_counts(),' distribution counts across the test set')
...
print("Some decoder input samples")
print(input_decoder_snippets[:4])
print("Some decoder output samples")
...
print("TOTAL TOKENS IN VOCAB:",len(set(vocab)))

word_counts = dict(Counter(vocab))
wcounts = list(word_counts.items())
wcounts.sort(key=lambda x: x[1], reverse=True)
print("The top 15 tokens sorted by the frequency count",wcounts[:15])

sorted_voc = []
sorted_voc.extend(wc[0] for wc in wcounts)
print("SORTED VOCAB KEYS:",sorted_voc[:15])
word_index = OrderedDict(zip(sorted_voc, list(range(1,1 + len(sorted_voc)))))

print("TOTAL WORDINDEX NUMBER:",len(word_index) + 1)
updated_word_index = OrderedDict(list(word_index.items())[:CODE_VOCAB_SIZE])
print("FINAL WORD INDEX USED",list(updated_word_index.items())[:15])
OOV = '<OOV>'
...
print("INTENTS samples:",input_encoder_intents[:4])

tok_intent.fit_on_texts(input_encoder_intents)


with open("model_files/tok-intent.pkl",'wb') as f:
  pickle.dump(tok_intent,f)

with open("model_files/tok-snippet.pkl",'wb') as f:
  pickle.dump(word_index,f)

input_decoder_snippets = df['clean_snippet'].apply(lambda snippet: ["<START>"] + snippet).values.tolist()
output_decoder_snippets = df['clean_snippet'].apply(lambda snippet: snippet + ["<END>"]).values.tolist()

print("Decoder input snippets",input_decoder_snippets[:3])
print("Decoder output snippets",output_decoder_snippets[:3])
...
print("VERIFY THE TRAIN, VAL and TEST sizes")
...
print("Intent sample",train_intent_sequences[0])

print("input decoder sample",train_input_snippet_sequences[0])

print('output decoder sample',train_output_snippet_sequences[0])
...
print("ENCODER INPUT INTENTS sizes")
...
print("DECODER INPUT INTENTS sizes")
...
print("DECODER OUTPUT INTENTS sizes")
...
print("TOTAL INTENT VOCAB",num_words_intent)
...
print("NUMBER NON VOCAB WORDS:",how_many_non_vocab)
...
print("Words not in the W2V vocab")
print(how_many_non_vocab)
...
decoder_layer = model.layers[-2]

_,h_out,c_out = decoder_layer.output
...
keras.utils.plot_model(model,show_shapes=True,show_layer_names=True,to_file="model_files/seq2seqmodel.png")
...
keras.utils.plot_model(binary_model,show_layer_names=True,show_shapes=True,to_file="model_files/binary_model.png")

"""#**FITTING models**

## **Fitting Seq2seq**
"""
print("TRAIN,VAL,TEST sizes for the positive samples")
where_train_positive = np.where(np.array(y_train) == 1)[0]
print(len(where_train_positive))

where_val_positive = np.where(np.array(y_val) == 1)[0]
print(len(where_val_positive))

where_test_positive = np.where(np.array(y_test) == 1)[0]
print(len(where_test_positive))
...
for i, d in enumerate(train_padded_output_snippet_sequences):
    for t, word in enumerate(d):
      decoder_targets_train_one_hot[i, t, word] = 1

for i, d in enumerate(test_padded_output_snippet_sequences):
    for t, word in enumerate(d):
        decoder_targets_test_one_hot[i, t, word] = 1

for i, d in enumerate(val_padded_output_snippet_sequences):
    for t, word in enumerate(d):
      decoder_targets_val_one_hot[i, t, word] = 1


print("Decoder output OHE shapes")
print(decoder_targets_train_one_hot.shape)
print(decoder_targets_test_one_hot.shape)
print(decoder_targets_val_one_hot.shape)
...
seq2seqmodel_history = model.fit(
          [train_padded_intent_sequences[where_train_positive][:], train_padded_input_snippet_sequences[where_train_positive][:]],
          decoder_targets_train_one_hot[where_train_positive][:][:],
          validation_data= ([val_padded_intent_sequences[where_val_positive][:],
            val_padded_input_snippet_sequences[where_val_positive][:]],
            decoder_targets_val_one_hot[where_val_positive][:][:]),
          epochs=EPOCHS,
          batch_size = 256
          )
...
with open('model_files/seq2seq_trainHistoryDict.pkl', 'wb') as file_pi:
  pickle.dump(seq2seqmodel_history.history, file_pi)

# serialize model to YAML
model_yaml = model.to_yaml()
with open("model_files/seq2seq_model.yaml", "w") as yaml_file:
    yaml_file.write(model_yaml)
# serialize weights to HDF5
model.save_weights("model_files/seq2seq_model.h5")
print("Saved model to disk")



yaml_file = open('model_files/seq2seq_model.yaml', 'r')
loaded_model_yaml = yaml_file.read()
yaml_file.close()
loaded_model = keras.models.model_from_yaml(loaded_model_yaml)
# load weights into new model
loaded_model.load_weights("model_files/seq2seq_model.h5")
print("Loaded model from disk")


with open("model_files/seq2seq_trainHistoryDict.pkl",'rb') as f:
  history = pickle.load(f)


#Load the models and the training history, 
'''
1. Plot train,val loss and acc for seq2seq
2. evaluate the testset
3. run the code generate() function and intent_w2v() for the train/test/val again for the binary!
4. run the binary model.
5. save binary model
6. then obtain all the needed results
'''
print()
loaded_model.compile(optimizer="Adam", loss="categorical_crossentropy",metrics = ['accuracy']) # Set up model
scores = loaded_model.evaluate([test_padded_intent_sequences[where_test_positive][:],test_padded_input_snippet_sequences[where_test_positive][:]],decoder_targets_test_one_hot[where_test_positive][:][:])
print("SEQ2SEQ")
...
print("Train out creation starts")
...
h,c = train_out[0][0][1],train_out[0][0][2]

print("train embedding creating")
train_code_embeds = np.multiply(h,c)
...
val_code_embeds = np.multiply(h,c)
...
print("OUTPUT SAMPLE DISTRIBUTION")
print(dict(Counter(y_test[:1000])))

test_out = hidden_layer_function([[test_padded_intent_sequences[:1000][:],test_padded_input_snippet_sequences[:1000][:]],decoder_targets_test_one_hot[:1000][:][:]])
h,c = test_out[0][0][1],test_out[0][0][2]

test_intent_embeds = np.zeros((1000,EMBEDDING_SIZE))
test_code_embeds = np.multiply(h,c)
i = 0
print("Testing code and intents")
for intent in test_intent_sequences[:1000]:
  intent_embed = intent_w2v(intent)
...
print("OUTPUT INTENT\tCODE EMBED sizes")
...
with open('model_files/binary_trainHistoryDict.pkl', 'wb') as file_pi:
  pickle.dump(bin_history.history, file_pi)

# serialize model to YAML
model_yaml = binary_model.to_yaml()
with open("model_files/binary_model.yaml", "w") as yaml_file:
    yaml_file.write(model_yaml)
# serialize weights to HDF5
binary_model.save_weights("model_files/binary_model.h5")
print("Saved model to disk")


scores = binary_model.evaluate([test_intent_embeds,test_code_embeds],np.array(y_test[:1000]))
...
with open("model_files/y_true.pkl",'wb') as true:
  pickle.dump(y_test[:1000],true)

with open("model_files/y_pred.pkl",'wb') as pred:
...
# sample_intent = test_padded_intent_sequences[0][:]
# sample_dec_input = test_padded_input_snippet_sequences[0][:]
# sample_dec_output = test_padded_output_snippet_sequences[0][:]
