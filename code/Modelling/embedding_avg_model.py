...
def code_generate(input_seq,_code_embedding_matrix = code_embedding_matrix):
    ...
    states_value = encoder_model.predict([input_seq]) #predict the hidden states from encoder first
    #we get a matrix of h,c values, so we pass only the last states
    ...
    target_seq[0, 0] = word2idx_outputs['<START>'] #start with the START token
    ...
    for _ in range(pad_length_snippet):
        ...
        if end_token == idx:
             return ' '.join(output_sentence),avg_embedding #if we hit the end token, stop the prediction

        word = ''

        if idx > 0:
            word = idx2word_target[idx] #in case of non empty prepadding token, get the word
            output_sentence.append(word)
            vector = _code_embedding_matrix[idx].reshape(-1,1) #get the embedding accordingly
            avg_embedding += vector #keep adding the embedding vector representation
            

        target_seq = np.zeros((1, 1))
        target_seq[0, 0] = idx
        states_value = [h, c] #update the state and move on
...
#function to get the intent embedding averaged W2V representation
...
#function to get the code embedding averaged W2V representation
def code_w2v(sequence, _embedding_matrix = code_embedding_matrix):
  #embedding matrix: idx to vector mapping (it is a matrix)
  avg_embed = np.zeros((EMBEDDING_SIZE,1))
  num_nonzero = 0
  for token in sequence:
    #get the embedding for the nonzero predicted token
    if token != 0:
      vector = _embedding_matrix[token].reshape(-1,1)
      avg_embed += vector
      num_nonzero += 1

  np.seterr(divide='ignore', invalid='ignore')#ADDED as a check for ignoring any NAN errors if we face
  return avg_embed/num_nonzero

#create the inputs for the binary model
#TRAIN SET
...
for i,intent in enumerate(train_intent_sequences):
  ...
  code = train_padded_input_snippet_sequences[i][1:] #predict from the first true token (ignore start)
  #in case of no NAN value; find the true average representation of the code sequence
  if np.isnan(code_w2v(code)).any() == False:
...
print("Binary Model Saved model to disk")
...
#Predict only for the 1000 samples in the binary classifier case
test_intent_embeds = np.zeros((BIN_NUM_TEST,EMBEDDING_SIZE))
test_code_embeds = np.zeros((BIN_NUM_TEST,EMBEDDING_SIZE))
test_code_outputs = []
y_test_sample = y_test[:BIN_NUM_TEST]
i = 0
